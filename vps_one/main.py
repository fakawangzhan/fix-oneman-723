...
async def dashboard(request: Request, db=Depends(session)):
    ...
    if instances:
        ...
        if base_url and token:
            try:
                remote_items = container_items(await CLICD(base_url, token).containers())
                for item in remote_items:
                    for key in (item.get("uuid"), item.get("id"), item.get("container_id")):
                        if key not in {None, ""}:
                            remote_by_id[str(key)] = item
                for instance in instances:
                    remote = remote_by_id.get(str(instance.clicd_id))
                    if remote:
    ...
    cards = [instance_card(instance, order_by_id[instance.order_id], plans[instance.plan_id], remote_by_id.get(str(instance.clicd_id))) for instance in instances if instance.order_id in order_by_id and instance.plan_id in plans]
